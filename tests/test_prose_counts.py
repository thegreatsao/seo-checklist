...
The registry's own populations — its size, the Plerdy split, the sources, the scripts,
the launches, the script-backed assertions — are not counted here. They come from
`test_protocol_counts.population()`, the one derivation of what `checklist.json`
decides; this module derives what lives elsewhere (the known-issues record, the
oracle, the census, the threshold and reachability audits, the specification's debt)
and binds the sentences that state either kind. A sentence is bound by one of the two
modules, never both: `test_no_number_is_bound_by_two_claims` reads both.
"""
...
import test_protocol_counts as protocol  # noqa: E402
...
def population(key: str) -> Callable[[], int]:
    return lambda: populations()[key]


def registry_plus_one() -> int:
    return populations()["items"] + 1
...
def assertion_counts() -> dict[str, int]:
    total = populations()["asserted"]
    proved = len(audit_reachability.proofs())
    return {"total": total, "proved": proved, "unclaimed": total - proved}
...
def source(name: str) -> Callable[[], int]:
    return population(f"source:{name}")
...
CLAIMS = (
    ...
    Claim("README.md", "unique launches", r"collapse to \*\*(?P<n>\d+) unique process launches", population("launches")),
    Claim("README.md", "launch scripts", r"unique process launches\*\* over (?P<n>\d+) distinct scripts", population("named_scripts")),
    Claim("README.md", "non-script items", r"(?P<n>\d+) items cannot be settled by a script", source("llm")),
    Claim("README.md", "per-item translations", r"(?P<n>\d+) would drift out of", population("items")),
    ...
    Claim(".claude-plugin/plugin.json", "description registry size", r"fixed (?P<n>\d+)-item registry", population("items")),
    Claim(".claude-plugin/plugin.json", "description Plerdy items", r"Plerdy (?P<n>\d+) \+", population("borrowed")),
    Claim(".claude-plugin/plugin.json", "description extra items", r"Plerdy \d+ \+ (?P<n>\d+)", population("added")),
    Claim(".claude-plugin/plugin.json", "description evidence scripts", r"(?P<n>\d+) evidence scripts", population("named_scripts")),
    Claim(".claude-plugin/plugin.json", "description judgement agents", r"(?P<n>\d+) judgement agents", lambda: agent_counts()[0]),
    Claim("pyproject.toml", "description registry size", r"fixed (?P<n>\d+)-item registry", population("items")),
    ...
    Claim("KNOWN-ISSUES.md", "census registry size", r"records what all (?P<n>\d+) items answered", population("items")),

    Claim("tests/corpus/README.md", "corpus declared items", r"has reached (?P<n>\d+) of \d+ items", declared_items),
    Claim("tests/corpus/README.md", "corpus registry size", r"has reached \d+ of (?P<n>\d+) items", population("items")),
    ...
    Claim("tests/verdict_census.py", "census doc registry size", r"has declared \d+ of (?P<n>\d+) items", population("items")),
    Claim("tests/verdict_census.py", "census all-at-once size", r"for all (?P<n>\d+) at once", population("items")),
    ...
    Claim("ROADMAP.md", "registry-size limit", r"(?P<n>\d+) is already past", population("items")),
...
def text_of(relative: str) -> str:
    with open(path_of(relative), encoding="utf-8") as stream:
        return stream.read()


def bound_spans() -> list[tuple[str, int, str]]:
    """Where every number this module reads sits: (file, offset, which claim)."""
    spans = [(path_of(claim.path), found.start("n"), claim.name)
             for claim in CLAIMS
             for found in re.finditer(claim.pattern, text_of(claim.path), re.MULTILINE)]
    spans += [(path_of("ROADMAP.md"), found.start("n"), "never-pass ids")
              for found in re.finditer(NEVER_PASS, text_of("ROADMAP.md"), re.DOTALL)]
    return spans
...
class ProseCountsDescribeThisTree(unittest.TestCase):
    ...
    def test_every_bound_count_is_present_once_and_equals_its_derivation(self):
        """One subtest per claim, so a registry move that shifts five sentences
        reports five, not the first of them."""
        for claim in CLAIMS:
            with self.subTest(file=claim.path, claim=claim.name):
                found = list(re.finditer(claim.pattern, text_of(claim.path), re.MULTILINE))
                self.assertEqual(
    ...
    def test_no_number_is_bound_by_two_claims(self):
        """A number two claims read is a number kept in step twice, and the two
        derivations behind it need not agree on what they count. Until this test
        `SKILL.md`'s registry size and added-item count and `README.md`'s assertion and
        evidence-script counts were bound here and in `test_protocol_counts` both, the
        assertions by two different derivations."""
        owners: dict[tuple[str, int], str] = {}
        doubled = []
        for module, spans in (("test_protocol_counts", protocol.bound_spans()),
                              ("test_prose_counts", bound_spans())):
            for path, offset, claim in spans:
                where = (os.path.normcase(os.path.abspath(path)), offset)
                here = f"{module} [{claim}]"
                if where in owners:
                    doubled.append(f"{os.path.relpath(path, ROOT)} at {offset}: "
                                   f"{owners[where]} and {here}")
                owners.setdefault(where, here)
        self.assertEqual(doubled, [], "a number is bound by two claims")

    def test_the_never_pass_sentence_names_exactly_the_census_set(self):
        found = list(re.finditer(NEVER_PASS, text_of("ROADMAP.md"), re.DOTALL))
        self.assertEqual(
            len(found), 1,
            f"ROADMAP.md [never-pass ids]: expected 1 regex match, found {len(found)}")
        match = found[0]
        stated = protocol.to_int(match.group("n"))
